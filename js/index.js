...
const displayBrands = brands => {
  ...
  for (brand of brands) {
    ...
    col.classList.add('col', 'd-flex', 'justify-content-center', 'border', 'rounded', 'border-dark', 'mx-1', 'mx-sm-2','p-0');
    col.setAttribute('onclick',`showAllPhoneOfClickedBrand('${brand.url}')`)
     col.setAttribute('id',`${brand.url}`)
    const card = document.createElement('div');
    card.classList.add('card', 'border-0','bg-transparent');
    card.innerHTML = `
            <div class="card-body bg-transparent">
              <p class="card-title text-center fw-bold"  id="${brand.url}" >${brand.name}</p>            
            </div>
            `;
